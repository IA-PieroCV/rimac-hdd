...
from config import (PIPELINE_ROOT, PIPELINE_NAME, BQ_INPUT_DATA, 
                    ...
                    SERVICE_ACCOUNT,
                    PROJECT_ID, REGION, CLASS_NAMES, TARGET_COLUMN,DATAFLOW_PUBLIC_IPS, DATAFLOW_SA, 
...
def pipeline(
    bq_table: str = "",
    sk_param_max_depth: int=10,
    sk_param_n_estimators: int=200,
    serving_container_image_uri: str = PRED_CONTAINER,    
):
    
    load_data_op = get_dataframe(
        bq_table=bq_table, 
        project_id=PROJECT_ID).set_display_name("Load And Split Data")
...
aiplatform.init(project=PROJECT_ID, location=REGION,
                #encryption_spec_key_name=KEY_ID
                )
...
if not args.compile_only:
    run = aiplatform.PipelineJob(
        ...
        parameter_values={
            ...
            "sk_param_max_depth": 5,
            "sk_param_n_estimators": 20},
